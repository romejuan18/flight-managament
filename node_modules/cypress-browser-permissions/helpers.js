...
function toPermissionState(value) {
    const maybeNumber = lodash_1.toNumber(value);
    if (lodash_1.isNumber(maybeNumber) && maybeNumber >= 0 && maybeNumber <= 2) {
        return maybeNumber;
    }
...
function getBrowserPermissionsFromEnv(env) {
    const permissionsRecord = lodash_1.chain(env[constants_1.PLUGIN_ENV_VAR])
        .mapKeys((_, key) => lodash_1.toLower(key))
